...
Memanggil dan menampilkan data set. Karena dari data wine ini mempunyai banyak data maka kita ambil n=10
```{r WARNING = FALSE}
data
data_wine = data
head (data_wine,n=10)
...
Memanggil data dengan menentukan nilai cetroid, dengan pembagian K=3

```{r WARNING = FALSE}
library(ppclust)
res.fcm <- fcm(data_wine, centers=3)
```
...
```{r WARNING = FALSE}
library(ppclust)
res.fcm <- fcm(data_wine, centers=3, nstart=5)
...
Cluster plot menggunakan pairwise scatter plots
```{r}
library (ppclust)
